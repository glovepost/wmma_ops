...
for spec in \
    down4:-4 down3:-3 down2:-2 down1:-1 \
    up1:1 up2:2 up3:3 up4:4; do
    name=${spec%%:*}
    delta=${spec##*:}
    candidate="bp-hot-fragment-${name}"
    python3 tools/rotate_hot_fragment_phase_asm.py \
        traces/bp-publish-lgkm0.s "traces/${candidate}.s" "${delta}"
