...
for mode in wmma-interleave store-interleave stores-first \
            stores-first-lgkm stores-first-no-wait; do
    candidate="bp-early-barrier-${mode}"
    python3 tools/patch_early_read_barrier_asm.py \
        traces/bp-register-phase-d2.s "traces/${candidate}.s" "${mode}"
