...
for alignment in 64 128 256 512; do
    candidate="bp-register-align-${alignment}"
    python3 tools/align_hot_loop_asm.py \
        traces/bp-register-phase-d2.s "traces/${candidate}.s" "${alignment}"
...
for padding in 8 24 40 56; do
    candidate="bp-register-pad-${padding}"
    python3 tools/pad_hot_loop_asm.py \
        traces/bp-register-phase-d2.s "traces/${candidate}.s" "${padding}"
    "${llvm}/clang" -target amdgcn-amd-amdhsa -mcpu=gfx1151 \
        -mcode-object-version=6 -c "traces/${candidate}.s" \
        -o "traces/${candidate}.o"
    "${llvm}/lld" -flavor gnu -m elf64_amdgpu --no-undefined -shared \
