...
build_candidate() {
    local candidate=$1
    shift
    python3 tools/patch_descriptor_mode_asm.py \
    ...
    echo "${candidate}:"
    grep -E '^\s+\.amdhsa_(memory_ordered|forward_progress)' \
        "traces/${candidate}.s"
    grep -E '^    \.(group_segment_fixed_size|sgpr_count|vgpr_count):' \
        "traces/${candidate}.s"
}

build_candidate bp-sched-oldest --forward-progress 0
build_candidate bp-mem-unordered --memory-ordered 0
build_candidate bp-sched-oldest-mem-unordered \
    --forward-progress 0 --memory-ordered 0
