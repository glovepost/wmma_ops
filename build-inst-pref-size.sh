...
build_candidate() {
    local size=$1
    local candidate="bp-inst-pref-size-${size}"
    python3 tools/patch_inst_pref_size_asm.py \
        traces/bp-publish-lgkm0.s "traces/${candidate}.s" "${size}"
    ...
    echo "${candidate}:"
    grep -E 'amdhsa_inst_pref_size|^    \.(group_segment_fixed_size|sgpr_count|vgpr_count):' \
        "traces/${candidate}.s"
}

for size in 0 8 12 16 32; do
    build_candidate "${size}"
done
