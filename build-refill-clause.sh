...
build_candidate() {
    local mode=$1
    local candidate="bp-refill-clause-${mode}"
    python3 tools/patch_refill_clause_asm.py \
        traces/bp-publish-lgkm0.s "traces/${candidate}.s" "${mode}"
...
build_candidate all-three
build_candidate nop
build_candidate drop
